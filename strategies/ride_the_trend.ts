...
export class RideTheTrend extends Strategy {
  ...
  evaluate(data: DataEntry): Array<TradeSuggestion> {
    ...
    let short_average = average(
      this.history
        .slice(this.history.length - this.short, this.history.length)
        .map((d) => d.priceOfB)
    );
    let long_average = average(this.history.map((d) => d.priceOfB));

    logger.info(
      { pool: this.pool, value: short_average, range: this.short },
      "moving average"
    );
    logger.info(
      { pool: this.pool, value: long_average, range: this.long },
      "moving average"
    );
    ...
    if (short_average != long_average && this.lastDecision > this.short + 1) {
      ...
      if (short_average / long_average > this.limit && !this.shortWasHigher) {
        ...
        short_average / long_average < 1 / this.limit &&
        this.shortWasHigher
      ) {
